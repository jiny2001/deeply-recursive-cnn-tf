...
class SuperResolution:
  ...
  def build_embedding_graph(self):
    ...
    self.H_conv[0] = util.conv2d_with_bias(Hm1_conv, self.W0_conv, self.cnn_stride, self.B0_conv, add_relu=True,  name="H0")
  ...
  def build_reconstruction_graph(self):
    ...
    for i in range(0, self.inference_depth+1):
      self.Y1_conv[i] = util.conv2d_with_bias(self.H_conv[i], self.WD1_conv, self.cnn_stride, self.BD1_conv, add_relu=not self.residual, name="Y%d_1"%i)
      self.Y2_conv[i] = util.conv2d_with_bias(self.Y1_conv[i], self.WD2_conv, self.cnn_stride, self.BD2_conv, add_relu=not self.residual, name="Y%d_2"%i)
